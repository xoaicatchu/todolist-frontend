...
export class TodoEffects {
  ...
  reorder$ = createEffect(() => this.actions$.pipe(
    ofType(TodoActions.reorder),
    switchMap(({ dayKey, orderedIds }) => from(this.es.append({ type: 'TODO_REORDERED', todoId: orderedIds[0] ?? crypto.randomUUID(), payload: { dayKey, orderedIds } }))),
    tap(() => {
      this.tabRealtime.notifyChanged();
      void this.sync.sync();
    }),
